...
func billInvoiceRules() *rules.Set {
	return rules.For(new(bill.Invoice),
		...
		rules.When(
			...
			rules.Field("tax",
				rules.Field("ext",
					rules.Assert("04", "invoice tax ext "+ExtKeyB2CCategory.String()+" is required on B2C invoices (G1.68)",
		...
		rules.When(
			...
			rules.Field("tax",
				rules.Field("ext",
					rules.Assert("08", "invoice tax ext "+untdid.ExtKeyDocumentType.String()+" must be one of the Flow 10 permitted UNTDID 1001 codes",
						tax.ExtensionsHasCodes(untdid.ExtKeyDocumentType, allowedDocumentTypes...),
					),
					rules.Assert("09", "invoice tax ext "+dgfip.ExtKeyBillingMode.String()+" is required and must be a valid billing-mode code (G1.02)",
			...
			rules.When(
				...
				rules.Field("tax",
					rules.Field("ext",
						rules.Assert("10", "invoice tax ext "+untdid.ExtKeyDocumentType.String()+" must not be an advance-payment code (386, 500, 503) when billing mode is final-after-advance (B4, S4, M4) (G1.60)",
