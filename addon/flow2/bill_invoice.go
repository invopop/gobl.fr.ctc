	...
	return rules.For(new(bill.Invoice),
		...
		rules.Field("supplier",
			rules.Field("endpoints",
				rules.Assert("11", "invoice supplier endpoint is required for French B2B invoices (BR-FR-13)",
					is.Present,
				),
			),
		...
		rules.When(
			...
			rules.Field("supplier",
				rules.Assert("13", "invoice supplier must have a Peppol endpoint with scheme 0225 matching the SIREN code (BR-FR-21/22)",
		...
		rules.Field("customer",
			rules.Field("endpoints",
				rules.Assert("14", "invoice customer endpoint is required for French B2B invoices (BR-FR-13)",
					is.Present,
				),
			),
			rules.Field("identities",
		...
		rules.When(
			...
			rules.Field("customer",
				rules.Assert("16", "invoice customer must have a Peppol endpoint with scheme 0225 matching the SIREN code (BR-FR-21/22)",
					is.Func("has SIREN endpoint", partyHasSIRENEndpoint),
				),
			),
		),
