	...
	return rules.For(new(bill.Invoice),
		...
		rules.Field("tax",
			...
			rules.Field("ext",
				...
				rules.Assert("09", "invoice tax ext fr-ctc-billing-mode is required and must be a valid billing-mode code",
		...
		rules.When(
			...
			rules.Field("customer",
				rules.Assert("45", "invoice customer must have an endpoint with scheme 0225 matching the SIREN code (BR-FR-21)",
		...
		rules.When(
			...
			rules.Field("supplier",
				rules.Assert("47", "invoice supplier must have an endpoint with scheme 0225 matching the SIREN code (BR-FR-22)",
					is.Func("has SIREN endpoint", partyHasSIRENEndpoint),
				),
			),
		),
