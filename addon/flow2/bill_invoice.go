...
func billInvoiceRules() *rules.Set {
	return rules.For(new(bill.Invoice),
		...
		rules.Field("tax",
			...
			rules.Field("ext",
				rules.Assert("08", "invoice tax ext "+untdid.ExtKeyDocumentType.String()+" must be a valid Flow 2 document type (BR-FR-04)",
					tax.ExtensionsHasCodes(untdid.ExtKeyDocumentType, allowedInvoiceDocumentTypes...),
				),
				rules.Assert("09", "invoice tax ext "+dgfip.ExtKeyBillingMode.String()+" is required and must be a valid billing-mode code",
		...
		rules.When(
			...
			rules.Field("tax",
				rules.Field("ext",
					rules.Assert("10", "invoice tax ext "+untdid.ExtKeyDocumentType.String()+" must not be an advance-payment code (386, 500, 503) when billing mode is factoring (B4, S4, M4) (BR-FR-CO-08)",
