...
func billInvoiceRules() *rules.Set {
	return rules.For(new(bill.Invoice),
			...
			rules.Field("preceding",
				...
				rules.Each(
					rules.Field("issue_date",
						rules.Assert("42", "invoice preceding issue_date is required for credit notes (BR-FR-CO-05)",
							is.Present,
						),
					),
				),
		...
		rules.Field("tax",
			...
			rules.Field("ext",
				rules.Assert("08", fmt.Sprintf("invoice tax ext %s must be a valid Flow 2 document type (BR-FR-04)", untdid.ExtKeyDocumentType),
					tax.ExtensionsHasCodes(untdid.ExtKeyDocumentType, allowedInvoiceDocumentTypes...),
				),
				rules.Assert("09", fmt.Sprintf("invoice tax ext %s is required and must be a valid billing-mode code", dgfip.ExtKeyBillingMode),
		...
		rules.When(
			...
			rules.Field("tax",
				rules.Field("ext",
					rules.Assert("10", fmt.Sprintf("invoice tax ext %s must not be an advance-payment code (386, 500, 503) when billing mode is factoring (B4, S4, M4) (BR-FR-CO-08)", untdid.ExtKeyDocumentType),
		...
		rules.Field("supplier",
			rules.Field("endpoints",
				rules.Assert("44", "invoice supplier endpoint is required for French B2B invoices (BR-FR-13)",
					is.Present,
				),
			),
			rules.Field("identities",
				rules.Assert("12", "invoice supplier legal identity must be a SIREN with iso-scheme-id 0002 (BR-FR-10/11)",
					is.Func("legal identity is SIREN", identitiesLegalIsSIREN),
				),
			),
		),
		rules.When(
			...
			rules.Field("customer",
				rules.Assert("45", "invoice customer must have an endpoint with scheme 0225 matching the SIREN code when the invoice is not self-billed (BR-FR-21)",
		...
		rules.Field("customer",
			rules.Field("endpoints",
				rules.Assert("46", "invoice customer endpoint is required for French B2B invoices (BR-FR-12)",
					is.Present,
				),
			),
			rules.Field("identities",
				rules.Assert("15", "invoice customer legal identity must be a SIREN with iso-scheme-id 0002 (BR-FR-10/11)",
					is.Func("legal identity is SIREN", identitiesLegalIsSIREN),
				),
			),
		),
		rules.When(
			...
			rules.Field("supplier",
				rules.Assert("47", "invoice supplier must have an endpoint with scheme 0225 matching the SIREN code when the invoice is self-billed (BR-FR-22)",
					is.Func("has SIREN endpoint", partyHasSIRENEndpoint),
				),
			),
		),
		...
		rules.Field("attachments",
			rules.Each(
				rules.Field("description",
					rules.AssertIfPresent("40", "invoice attachment description must be one of the allowed values (BR-FR-17)",
